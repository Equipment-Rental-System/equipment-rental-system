...
- 다른 PC에서는 경로가 다르면 직접 실행 방식으로 돌리는 것이 더 안전합니다.

## 실행 검증 시 환경별 설정

테스트 환경이 다르면 세팅해야 하는 항목이 조금씩 다릅니다.
...
- 에뮬레이터가 아니라 같은 머신 기준 테스트일 때 적합합니다.
...
5. `frontend`에서 `npm install`, `npx expo start`
6. `frontend/src/constants/appConstants.js`에 `10.0.2.2` 사용
...
4. `backend`에서 `npm install`, `npm start`
5. `frontend`에서 `npm install`, `npx expo start`
6. `frontend/src/constants/appConstants.js`에 `내PC내부IP` 사용
...
2. 프론트엔드는 `frontend/src/services/api.js`의 `signupAgainstBackend()`를 통해 `POST /api/signup` 요청을 `multipart/form-data`로 보냅니다.
...
1. 사용자가 `frontend/src/screens/LoginScreen.js` 화면에서 학번 또는 관리자 아이디와 비밀번호를 입력합니다.
...
- 프론트엔드: `frontend/src/screens/*`, `frontend/src/services/api.js`
- API 주소 관리: `frontend/src/constants/appConstants.js`
